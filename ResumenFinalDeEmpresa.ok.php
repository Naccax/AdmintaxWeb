  ...
  foreach($movimientos as $row)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          $ListaGV2[]=$row;
          //if((str_contains($row['Observaciones'],'GOMERIA:')) or (str_contains($row['Observaciones'],'Alineación')) or (str_contains($row['Observaciones'],'Rotación')) or (str_contains($row['Observaciones'],'GOMERÍA')))//GOMERÍA
          if((str_contains($row['Observaciones'],'GOMERIA:')) or (str_contains($row['Observaciones'],'GOMERÍA')))//GOMERÍA
          {
            $listaGomeria[]=$row;
          ...
          }elseif((str_contains($row['Observaciones'],'MANO DE OBRA:')) or (str_contains($row['Observaciones'],'MANO DE OBRA;')))//MANO DE OBRA;
          {
            $listaMonoDeObra[]=$row; 
...
//print_r($choferes[6]['Nombre']);
  ...
  //echo "Contador de otros".$contador['Otros'];
  ...
  $contadorGV2['Liquido']=0;
  if(isset($ListaGV2))
    foreach($ListaGV2 as $row)
    {
            $contadorGV2['Liquido']=$contadorGV2['Liquido']+$row['Liquido'];
    }
...
<div class="container text-center abs-center">
  ...
  <table class="table">
    <thead>
    <tr>
      <th>Detalle</td> <th>Monto</td><th width="1%">Axiones</td>
    </tr></thead>
    <tr>
      <!-- <td> Total Recaudado </td><td><?php echo "$ ".$contador['Recaudado']; ?></td> -->
      <td>INGRESO BRUTO (Recaudación + Aportes)</td><td><?php echo "$ ".$contador['Recaudado']+$contador['AporteReal']; ?></td>
    </tr>
    <tr>
      <td>TOTAL RECAUDADO: </td><td><?php echo "$ ".$contador['Recaudado']; ?></td>
    </tr>
    <tr>
      <td>Gastos Fijos</td><td><?php echo "$ ".$contadorGF['Liquido'] + $Administr; $totalizadorDeGastos=$totalizadorDeGastos+$contadorGF['Liquido'] + $Administr;?></td><td><button id="show" class="btn btn-success" style="align: right;">Detalles</button></td>
  <?php
    $acumuladorDeGastosFijos=0;
    $leasing=0;
    $seguros=0;
    $radiotaxiPatronal=0;
    $radiotaxiCeleritas=0;
    $lojack=0;
    foreach ($ListaGF as $row) 
    {
      //str_contains(string $row[], string $needle)
      
      if (str_contains($row['Observaciones'], 'LEASING: ')){
        $leasing=$leasing+$row['Liquido'];
      }
      elseif (str_contains($row['Observaciones'], 'SEGURO: ')){
        $seguros=$seguros+$row['Liquido'];
      }//RADIO TAXI PATRONAL: 
      elseif (str_contains($row['Observaciones'], 'RADIO TAXI PATRONAL: ')){
        $radiotaxiPatronal=$radiotaxiPatronal+$row['Liquido'];
      }
      elseif (str_contains($row['Observaciones'], 'RADIO TAXI CELERITAS: ')){
        $radiotaxiCeleritas=$radiotaxiCeleritas+$row['Liquido'];
      }
      elseif (str_contains($row['Observaciones'], 'LOJACK: ')){
        $lojack=$lojack+$row['Liquido'];
      }
      else{
        echo '<tr class="cat1" style="display:none">';
        echo '<td>'.str_replace("GF: ","",$row['Observaciones']).'</td><td>$ '.$row['Liquido'].'</td><td></td>';
        echo '</tr>';
      }
      $acumuladorDeGastosFijos=$acumuladorDeGastosFijos+$row['Liquido'];
    }
    
    echo '<tr class="cat1" style="display:none">';
    echo '<td>Administración </td><td>$ '.$Administr.'</td><td></td>';
    $acumuladorDeGastosFijos=$acumuladorDeGastosFijos+$Administr;
    echo '</tr>';

    
    echo '<tr class="cat1" style="display:none">';
    echo '<td>LEASING: </td><td>$ '.$leasing.'</td><td></td>';
    echo '</tr>';

    echo '<tr class="cat1" style="display:none">';
    echo '<td>SEGURO: </td><td>$ '.$seguros.'</td><td></td>';
    echo '</tr>';

    echo '<tr class="cat1" style="display:none">';
    echo '<td>RADIO TAXI PATRONAL: </td><td>$ '.$radiotaxiPatronal.'</td><td></td>';
    echo '</tr>';

    echo '<tr class="cat1" style="display:none">';
    echo '<td>RADIO TAXI CELERITAS: </td><td>$ '.$radiotaxiCeleritas.'</td><td></td>';
    echo '</tr>';
    
    echo '<tr class="cat1" style="display:none">';
    echo '<td>LOJACK: </td><td>$ '.$lojack.'</td><td></td>';
    echo '</tr>';


    echo '<tr class="cat1" style="display:none">';
    echo '<td><b>Total:</td><td><b>$ '.$acumuladorDeGastosFijos.'</td><td></td>';
    echo '</tr>';
    ...
    ?>
        ...
        <td></td>
        <td></td>
            <td><button id="hide" class="btn btn-danger">Ocultar</button></td>
        </tr>


    </tr>
    <tr>
      <td>Salarios más complementos de laudos y feriados.</td>
      <td><?php echo "$ ".$contador['Salario']+$contador['Laudo']+$contador['FeriadoNoCobrado']+$contador['FeriadoCobrado']; $totalizadorDeGastos=$totalizadorDeGastos+$contador['Salario']+$contador['Laudo']+$contador['FeriadoNoCobrado']+$contador['FeriadoCobrado'];?></td>
      <td><button id="show2" class="btn btn-success">Detalles</button></td>
    </tr>

        <tr class="cat2" style="display:none">
            <td>Feriados</td><td><?php echo "$ ".$contador['FeriadoNoCobrado']+$contador['FeriadoCobrado']; ?></td><td></td>
        </tr>
        <tr class="cat2" style="display:none">
            <td>Salarios</td><td><?php echo "$ ".$contador['Salario']; ?></td><td></td>
        </tr>
        <tr class="cat2" style="display:none">
            <td>Laudos</td><td><?php echo "$ ".$contador['Laudo']; ?></td><td><button id="show3" class="btn btn-warning">Detalles</button></td>
        </tr>
        <td colspan="2">
            <table width="100%" class="cat3" style="display:none">
                <tr class="cat3"><td><b>Fecha</td><td><b>Chofer</td><td><b>Móvil</td><td><b>Monto</td><td><b>Detalle</td></tr>

                <?php 
                foreach($ListaDeLaudos as $row)
                {
                    $nombre="Chofer NO encontrado";
                    $id=$row['Chofer'];
                    echo '<tr class="cat3" style="display:none">'.'<td>'.$row['HoraEntrada'].' </td><td>';
                    ...
                    if($nombre=="Chofer NO encontrado")
                    {
                        ...
                        {
                        // print_r($chofer);
                            $nombre=$chofer[0]["Nombre"];
                        }
                        catch (Exception $e)
                        {}
                    }
                    echo $nombre."</td><td>";
                    echo $row["Movil"]." </td><td> $ ". $row["Laudo"];
                    echo '</td><td>'.$row["Observaciones"].'</td>';
                    echo '</tr>';   
                }
                ?>
            </table>
            ...
            <tr class="cat3" style="display:none">
                <td></td><td></td>
                <td><button id="hide3" class="btn btn-danger">Ocultar</button></td>
            </tr>
        </div>
        <tr class="cat2" style="display:none">
            <td>Total: </td><td><?php echo "$ ".$contador['Salario']+$contador['Laudo']+$contador['FeriadoNoCobrado']+$contador['FeriadoCobrado']; ?></td>
        </tr>
        <tr class="cat2" style="display:none">
            <td></td><td></td>
            <td><button id="hide2" class="btn btn-danger">Ocultar</button></td>
        </tr>
    
    <tr>
      <td> Viáticos (Todos)</td><td><?php echo "$ ".$contador['ViaticoCobrado']+$contador['ViaticoNoCobrado']; $totalizadorDeGastos=$totalizadorDeGastos+$contador['ViaticoCobrado']+$contador['ViaticoNoCobrado'];//gggg?></td>
    </tr>
    <tr>
      <td> Gas-Oil Contado</td><td><?php echo "$ ".$contador['GasOilPlata']; $totalizadorDeGastos=$totalizadorDeGastos+$contador['GasOilPlata']+$contador['GasOilCred']+$contador['Lavado'];?></td>
    </tr>
    ...
    <tr>
      <td>Gomerias </td><td><?php echo "$ ".round($contador['Gomeria']+$gastosDeGomeria,2);$totalizadorDeGastos=$totalizadorDeGastos+$contador['Gomeria']+$gastosDeGomeria; ?></td><td><button id="show7" class="btn btn-success">Detalles</button></td>
    </tr>
  ...
  if(isset($listaGomeria))
    ...
    echo "</td>";
    echo "<td>";
    echo "<b>$ ".$gastosDegomería." <br>(Total: $ ".$contador["Gomeria"]+$gastosDegomería.")";
    echo "</td>";
    echo "</tr>";
    ...
    <tr class="cat7" style="display:none">
            <td></td><td></td>
            <td><button id="hide7" class="btn btn-danger">Ocultar</button></td>
    </tr>


    <tr>
      <td>Recargas Cabify</td><td><?php echo "$ ".round($contador['Cabify'],2); $totalizadorDeGastos=$totalizadorDeGastos+$contador['Cabify']+$contador['Otros'];?></td>
    </tr>
    <tr>
      <td>Otros Gastos en Boletas</td><td><?php echo "$ ".$contador['Otros']; ?></td><td><button id="show4" class="btn btn-success">Detalles</button></td>
    </tr>
    ...
    <thead>
    <tr>
    <th>Fecha</td><td>Móvil</td> <th>Detalle</th> <th>Monto</td>
    </tr></thead>
    <?php 
    //print_r($ListaGV);

    $aux=0;
    $conatdorHorasDeCaraga=0;
    
    foreach ($listaOtrosGastos as $row) 
    {
        if(($row["Observaciones"]=="Hora de carga")or($row["Observaciones"]=="Hora de Carga"))
        {
            $conatdorHorasDeCaraga=$conatdorHorasDeCaraga+$row['Otros'];
        }elseif(isset($row["HoraDeCargaCobrada"]))
        {
            $conatdorHorasDeCaraga=$conatdorHorasDeCaraga+$row["HoraDeCarga"];
        }elseif(isset($row["HoraDeCargaNoCobrada"]))
        {
            $conatdorHorasDeCaraga=$conatdorHorasDeCaraga+$row["HoraDeCarga"];
        }
        else
    ...
    {        
        ...
        echo "</td>";
        echo "<td>";
        echo $conatdorHorasDeCaraga;
        $aux=$aux+$conatdorHorasDeCaraga;
        echo "</td>";
        echo "</tr>";
    }
    ...
    <tr class="cat4" style="display:none">
            <td></td><td></td>
            <td><button id="hide4" class="btn btn-danger">Ocultar</button></td>
    </tr>

    <tr>
      <td> Gastos Variables</td><td><?php echo "$ ".$contadorGV['Liquido']*(-1); $totalizadorDeGastos=$totalizadorDeGastos+$contadorGV['Liquido']*(-1);?></td><td><button id="show5" class="btn btn-success">Detalles</button></td>
    </tr>
    <tr class="cat5" style="display:none">
        <td colspan='3' class="cat5" style="display:none">
        <table class="table">
    <thead>
    <tr>
    <th>Fecha</td> <th>móvil</th> <th>Detalle</th> <th>Monto</td>
  ...
  if(isset($ListaGV))
    ...
    {
      //echo "<tr>";//
      echo '<tr class="cat5" style="display:none">';
      echo "<td>";
      echo $row['fecha'];
      echo "</td>";
      echo "<td>";
      ...
      echo "</td>";
      echo "<td>";
      echo str_replace("GV: ","",$row['Observaciones']);
    ...
    <tr class="cat5" style="display:none">
            <td></td><td></td>
            <td><button id="hide5" class="btn btn-danger">Ocultar</button></td>
    </tr>


    
    <?php if($sumaAceites>0){?>

    <tr>
      <td> Gastos de Aceite</td><td><?php echo "$ ".$sumaAceites; $totalizadorDeGastos=$totalizadorDeGastos+$sumaAceites+$contador['Electicidad'];?></td>
    </tr>
    
    <?php }?>


    <?php if($contador['Electicidad']>0){?>
    <tr>
      <td> Electricidad Crédito</td><td><?php echo "$ ".round($contador['Electicidad'],2); ?></td>
    </tr>
    <?php }?>
      
    <?php if($gastosRepuestos>0){?>
    <tr>
      <td>Repuestos </td><td><?php echo "$ ".round($gastosRepuestos,2); $totalizadorDeGastos=$totalizadorDeGastos+$gastosRepuestos;?></td><td><button id="show6" class="btn btn-success">Detalles</button></td>
    </tr>
    
    <?php }?>

    <tr  class="cat6" style="display:none">
      <td colspan='3'  class="cat6" style="display:none">
  ...
  if(isset($ListaRepuesto))
    ...
    <tr class="cat6" style="display:none">
            <td></td><td></td>
            <td><button id="hide6" class="btn btn-danger">Ocultar</button></td>
    </tr>
    ...
    <tr>
      <td>Mano de obra </td><td><?php echo "$ ".round($gastosManoDeObra,2); $totalizadorDeGastos=$totalizadorDeGastos+$gastosManoDeObra;?></td><td><button id="show8" class="btn btn-success">Detalles</button></td>
    </tr>
  ...
  if(isset($listaMonoDeObra))
    ...
    <tr class='cat8' style="display:none">
            <td></td><td></td>
            <td><button id="hide8" class="btn btn-danger">Ocultar</button></td>
    </tr>
    ...
    <tr>
      <td> Pagos electrónicos</td><td><?php echo "$ ".$pagosElecrtonicos; ?></td><td><button id="show9" class="btn btn-success">Detalles</button></td>
    </tr>
    ...
    <tr class='cat9' style="display:none">
            <td></td><td></td>
            <td><button id="hide9" class="btn btn-danger">Ocultar</button></td>
    </tr>
    
    <tr class="Neutral">
      <td> Efectivo Real en el móvil</td><td><?php echo "$ ".$contador['Efectivo']+$contadorMV['Efectivo']+$contadorGV2['Liquido']-$contadorPA['Liquido']; ?></td>
    </tr>

    <tr>
      <td> EFECTIVO A ENTREGAR (Descontando Viáticos y Feriados)</td><td>
        <?php
        $aaa= $contador['Efectivo']-$contadorPA['Liquido']+$contadorMV['Liquido']+$contadorGV2['Liquido']-($contador['FeriadoNoCobrado']+$contador['ViaticoNoCobrado']);
        echo "$ ".round($aaa,2); ?></td>
    </tr>
    <tr>
      <td> Aportes Dejados Por Choferes</td><td><?php echo "$ ".$contador['AporteReal']; ?></td>
    </tr>
    <?php if($contadorPA['Liquido']>0){?>
    <tr class="filaAzul">
    ...
    <tr class="Neutral">
      <td> Ganancia Neta:</td><td>
        <?php
        echo $contador['Recaudado']-$totalizadorDeGastos;
        //$tempGanaciaNeta
        ?></td>
    </tr>
