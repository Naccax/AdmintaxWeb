...
$sql="SELECT * from retroactivos ORDER by fecha ASC;";
$parametros=ejecutarConsulta($sql);

foreach($parametros as $param)
{
    $sql="UPDATE recaudaciones SET ViaticoCorrespondiente='".$param["Monto"]."' WHERE HoraEntrada>='".$param["Fecha"]."';";
    Insert($sql);
}

$sql="UPDATE recaudaciones SET 
`DiferenciaDeViatico`=TRUNCATE((`ViaticoCorrespondiente`-(`ViaticoCobrado`+`ViaticoNoCobrado`)),2), 
`DiferenciaDeViaticoNeta`=TRUNCATE((`ViaticoCorrespondiente`-(`ViaticoCobrado`+`ViaticoNoCobrado`)),2)
WHERE HoraEntrada>='2022-07-01 00:00:00';";
Insert($sql);

$sql="UPDATE recaudaciones SET 
`DiferenciaDeViaticoNeta`=TRUNCATE(
...
$sql="UPDATE recaudaciones SET 
`RetencionDeRetroactividad`=TRUNCATE(
    ((DiferenciaDeViaticoNeta*0.35)*(OtraRetencion/100)
    ),2)
WHERE HoraEntrada>='2022-07-01 00:00:00' 
AND ViaticoNoCobrado>0;";
        ...
        <tbody>
            ...
            foreach($registros as $reg)
            {
                ...
                echo "
                    ...
                    <input type='Submit' class='btn btn-primary' name='Generar Recibo' value='Generar Recibo' title='Generar Recibo' width='30' height='15'>
                    </form>
                ";
                echo'</td>';
                echo'</tr>';
