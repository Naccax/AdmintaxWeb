...
$_POST["FonasaAplicado"]=$temp[0]["FonasaAplicado"];
...
$datos[0]["tipoAporte"]=$_POST["FonasaAplicado"];

$sql="SELECT * FROM planilladetrabajo WHERE Fecha='".$_POST["fecha"]."' AND Concepto like '%Aguinaldo%' AND ChoferID='".$_POST["ChoferID"]."' AND EmpresaID='".$_POST["EmpresaID"]."' and Id='".$_GET["rsid"]."';";
...
if(true)
{
  ...
  if($descuntoJuvilatorio=="23.1")
  {
    $descuntoJuvilatorio=20.100;
  }else
  {
    if($descuntoJuvilatorio=="19.6")
    {
      $descuntoJuvilatorio=18.1;  
    }
    else
    {
      $descuntoJuvilatorio=18.100;  
    }
...
$totalRecibido=$_POST["TotalBruto"]-$temp;
...
$descJubilatorio=round(($_POST["TotalBruto"]*0.15),2);
$descFonasa=$descuntoJuvilatorio-15.1;
$descFonasaMonto=round(($_POST["TotalBruto"]*($descFonasa/100)),2);
$descFrl=round(($_POST["TotalBruto"]*(0.001)),3);

$totalDescuentos=round( $_POST["TotalBruto"]*($descuntoJuvilatorio/100),2);

$totalRetenido=$retencion;
$totalRetenido=round($totalRetenido,2);

$sql="UPDATE planilladetrabajo SET Retencion='".$totalRetenido."' WHERE Id='".$_GET["rsid"]."';";
Insert($sql);
...
$reciboFecha="(".$meces[$temp[01]]." de ".$temp[0].")";
if($temp[01]<10)
{
  $aux=$temp[01]+1;
  $aux="0".$aux;
...
<table border="1" calss="ex">
    <tr>
    <td colspan="3"> <img src="./img/favicon.png" class="logo"> <B>AdminTax | &nbsp; LIQUIDACIÓN CORRESPONDIENTE AGUINALDO <?php echo $reciboFecha;?></td><td class="cons"><b>RECIBO DE AGUINALDO</td>
    </tr>
    <tr>
    <td width='48%'>
    ...
    <td colspan="3" calss="td2">
        <table border="0">
            <tr><td>Sueldo base para Aportación</td><td>$ <?php echo $_POST["TotalBruto"]; ?></td></tr>
            <tr><td>Alicuota Viático 50%</td><td>$ <?php echo 0; ?></td></tr>
            <tr><td>Total Para Aportación</td><td>$ <?php echo $_POST["TotalBruto"]; ?></td></tr>
        </table>
        <br>
        <table border="0">
