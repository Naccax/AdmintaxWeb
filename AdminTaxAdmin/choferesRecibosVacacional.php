...
{
  $sql="UPDATE planilladetrabajo SET LicenciaGozada='Si' WHERE Id='".$_GET["id"]."'";
  if($_GET["estado"]!="No")
  {
    $sql="UPDATE planilladetrabajo SET LicenciaGozada='No' WHERE Id='".$_GET["id"]."'";
  }
  Insert($sql);
}
...
$consulta="SELECT * FROM `laudos` WHERE `Detalle`='Jornal de 8 hs.' ORDER by `Fecha` DESC;";
$temp=ejecutarConsulta($consulta);
$jornalDe8hs=$temp[0]["Monto"];


$consulta="SELECT * FROM `laudos` WHERE `Detalle`='Jornal de 12 hs.' ORDER by `Fecha` DESC;";
$temp=ejecutarConsulta($consulta);
$jornalDe12hs=$temp[0]["Monto"];

function GetPromAnterior($chofer,$fecha)
{
    $sql="SELECT SueldoBaseParaAportacion,Comunes_Jornales,FeriadosNoLaborables_Cant,Concepto,Fecha 
    FROM `planilladetrabajo` 
    WHERE ChoferID=".$chofer."
    AND Concepto='Recibo Sueldo'
    and Fecha<='".$fecha."'
    ORDER BY planilladetrabajo.Fecha DESC;";
    $respuesta=ejecutarConsulta($sql);
    $promAnterior=($respuesta[0]["SueldoBaseParaAportacion"])/($respuesta[0]["Comunes_Jornales"]+$respuesta[0]["FeriadosNoLaborables_Cant"]);
    $promAnterior=$promAnterior*(0.954);
    $promAnterior=round($promAnterior,2);
...
max(`planilladetrabajo`.Id) as PlanillaId,
Truncate(sum(`planilladetrabajo`.Id),2) as Id,
Truncate(sum(`planilladetrabajo`.Comunes_MontoAbonado + `planilladetrabajo`.FeriadosNoLaborables_Monto),2) as Recaudacion,
Truncate(sum(`planilladetrabajo`.Comunes_MontoAbonado),2) as Recaudacion2,
Truncate(sum(`planilladetrabajo`.RemuneracionTotal),2) as RemuneracionTotal,
Truncate(sum(`planilladetrabajo`.Comunes_Jornales),2) as Comunes_Jornales,
Truncate(sum(`planilladetrabajo`.Salario29),2) as Salario29,
Truncate(sum(`planilladetrabajo`.FeriadosNoLaborables_Cant),2) as FeriadosNoLaborables_Cant,
Truncate(sum(`planilladetrabajo`.Salario29 + `planilladetrabajo`.FeriadosNoLaborables_Monto),2) as SueldoBaseParaAportacion,
`planilladetrabajo`.EmpresaID as EmpresaID,
`planilladetrabajo`.LicenciaGozada as LicenciaGozada,
licencias.id as id,
licencias.desde as desde,
        ...
        <tbody>
            <?php
                ...
                foreach($docs as $doc)
                {
                    ...
                    $recibosDeAguinaldos[$i]['LicenciaGozada']=$doc['LicenciaGozada'];
                    echo '</td>';
                    echo '<td>';
                    $licencia=0;

                    $promedioAnterior=GetPromAnterior($doc['ChoferID'],$doc['hasta']);
                    $licencia=round($promedioAnterior,2);
                    
                    $licencia=$licencia*$jornales;
                    $licencia=round($licencia,2);
                    echo $licencia;
                    $recibosDeAguinaldos[$i]['LicenciaPorAportacion']=$licencia;
                    echo '</td>';
                    if($masInfo){
                        echo '<td>';
                        echo $doc['Salario29'];
                    ...
                    <td>
                        <form action="recibo_VacacionalyLicencia_pdf.php" method="post">
                            ...
                            <input type="text" name="LicenciasId" Value="'.$doc['id'].'" hidden>
                            ...
                            <input type="text" name="LicenciaGozada" Value="'.$recibosDeAguinaldos[$i]['LicenciaGozada'].'" hidden>
                            <input type="text" name="LicenciaPorAportacion" Value="'.$recibosDeAguinaldos[$i]['LicenciaPorAportacion'].'" hidden>
                            <input type="text" name="Comunes_Jornales" Value="'.$recibosDeAguinaldos[$i]['Comunes_Jornales'].'" hidden>
...
foreach($recibosDeAguinaldos as $recibo)
{
    ...
    if($recibo['Id']=="0")
    {
        $sql="INSERT INTO `planilladetrabajo` (Fecha,Concepto,EmpresaID,ChoferID,TotalBruto,SueldoBaseParaAportacion,Comunes_Jornales) VALUES ";
        $sql=$sql."('".$recibo['Fecha']."','".$recibo['Concepto']."','".$recibo['EmpresaID']."','".$recibo['ChoferID']."','".$recibo['TotalBruto']."','0','".$recibo['Comunes_Jornales']."');";
    }
    else
    {
