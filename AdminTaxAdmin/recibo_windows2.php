...
if (isset($_GET["rsid"]))
{
  $sql="SELECT *  FROM planilladetrabajofueradenomina WHERE Id='".$_GET["rsid"]."'";
  $planilladetrabajofueradenomina=ejecutarConsulta($sql);
}

if($planilladetrabajofueradenomina[0]["Concepto"]=="Licencia Aportación")
{
  $planilladetrabajofueradenomina[0]["Concepto"]="Salario Vacacional y Licencia";
}
elseif($planilladetrabajofueradenomina[0]["Concepto"]=="Aguinaldo Aportación")
{
  $planilladetrabajofueradenomina[0]["Concepto"]="Aguinaldo";
}
/*
if ((isset($_GET["f"])) AND ($planilladetrabajofueradenomina[0]["FonasaAplicado"]=="0"))
{
  $planilladetrabajofueradenomina[0]["FonasaAplicado"]=$_GET["f"];
  $update="UPDATE planilladetrabajofueradenomina SET  FonasaAplicado = '".$_GET["f"]."' WHERE Id='".$_GET["rsid"]."'";
  update($update);
}*/

//EmpresaDatos
$sql="SELECT * FROM empresa WHERE id='".$planilladetrabajofueradenomina[0]["EmpresaID"]."';";
$EmpresaDatos=ejecutarConsulta($sql);

//ChoferDatos
$sql="SELECT * FROM chofer WHERE Id='".$planilladetrabajofueradenomina[0]["ChoferID"]."';";
$ChoferDatos=ejecutarConsulta($sql);

$fechaArray = explode('-',$planilladetrabajofueradenomina[0]["Fecha"]);
$fechaLiquidacion=$meses[$fechaArray[1]].' / '.$fechaArray[0];
$fechaLiquidacion2=$fechaLiquidacion;

if($planilladetrabajofueradenomina[0]["Concepto"]=="Salario Vacacional y Licencia")
{
  $fechaArray2 = explode('-',$planilladetrabajofueradenomina[0]["Fecha"]);
  $fechaLiquidacion2=$fechaArray2[0];
}


$fechaArray3 = explode('-',$planilladetrabajofueradenomina[0]["GozadaFecha"]);
@$fechaLiquidacion3=$meses[$fechaArray3[1]].' / '.$fechaArray3[0];


if($planilladetrabajofueradenomina[0]["Concepto"]=="Aguinaldo")
{
  $fechaLiquidacion2=$fechaLiquidacion3;
}

//Detalles de Recibo 
$sql="SELECT * FROM `recibodetallerw2` WHERE `IdRs`=".$_GET["rsid"]." order BY `Orden`;";
$recibodetallerw2=ejecutarConsulta($sql);
...
foreach($recibodetallerw2 as $row)
{
  if ($row["Cantidad"]>=0)
  {
...
$descFonasa=$planilladetrabajofueradenomina[0]["FonasaAplicado"]-15.1;
...
$liquidoPorViaticos=$liquidoPorViaticos*($planilladetrabajofueradenomina[0]["FonasaAplicado"]/100);
...
<table border="1" calss="ex">
    <tr>
    <td colspan="3"> <img src="./img/favicon.png" class="logo"> <B>AdminTax | &nbsp; LIQUIDACIÓN CORRESPONDIENTE:  <?php echo $planilladetrabajofueradenomina[0]["Concepto"]." (".$fechaLiquidacion2.")";?></td><td class="cons"><b><?php echo $planilladetrabajofueradenomina[0]["Concepto"];?></td>
    </tr>
    <tr>
    <td width='48%'>
    ...
    <td colspan="3" calss="td2">
        ...
        <table border="0">
            ...
            <tr><td>IRPF (Anticpo)(Imp. $<?php echo $totalParaAportacion; ?>)</td><td>0</td><td>$ <?php echo $planilladetrabajofueradenomina[0]["IRPF"]; ?></td></tr>
